...
public class RecipeSeeder implements Seeder {
  ...
  public void seed() {
    if (recipeRepository.count() == 0) {
      ...
      if (users.isEmpty()) {
        System.out.println(
            ">> Warning: No users found to associate with recipes. Skipping recipe seeder.");
        return;
      }
      ...
      IntStream.range(0, 20)
          .forEach(
              i -> {
                ...
                // On génère un titre de plat en Français (le faker locale "fr" gère déjà bien
                // food().dish())
                ...
                Recipe recipe =
                    Recipe.builder()
                        ...
                        .title(dishName)
                        .description(
                            "Une délicieuse préparation traditionnelle. "
                ...
                IntStream.rangeClosed(1, nbSteps)
                    .forEach(
                        stepNumber -> {
                          ...
                          String stepDescription =
                              String.format(
                                  "%s %s. %s", action, ingredient, faker.lorem().sentence());
  ...
  @Override
  public int getOrder() {
    return 4; // Doit être exécuté APRES UserSeeder (qui a l'ordre 3) car les recettes ont besoin
    // d'un User
  }
}

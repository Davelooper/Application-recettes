  ...
  public void seed() {
    if (recipeIngredientRepository.count() == 0) {
      ...
      if (recipes.isEmpty() || allIngredients.isEmpty()) {
        System.out.println(
            ">> Warning: Missing recipes or ingredients to seed recipe_ingredients. Skipping.");
        return;
      }
      ...
      recipes.forEach(
          recipe -> {
            ...
            // On mélange la liste des ingrédients pour en prendre au hasard sans doublon pour cette
            // recette
            ...
            recipeIngredients.forEach(
                ingredient -> {
                  // Choisir une unité au hasard (peut être null pour certains cas, mais on va en
                  // forcer une pour l'exemple)
                  ...
                  if (unitName.contains("pincée")
                      || unitName.contains("gousse")
                      || unitName.contains("cuillère")) {
